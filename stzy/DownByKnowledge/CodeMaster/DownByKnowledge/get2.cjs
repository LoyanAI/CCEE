...
const fs = require('fs')
const db = new sqlite3.Database(path.join(__dirname, "stzysk5.db"));
...
async function getter(treeid) {
    try {
        var gl = [];
        for (var i1 = 13; i1 < LprovinceCode.length; i1++) {
                        ...
                        LpaperType[i8].forEach(PapperType => {
                            gl.push(
                                new Promise((resolve, reject) => {
                                    process.stdout.write(`\r${i1 + 1}/33 ${i5 + 1}/6 ${i7 + 1}/9 ${i8 + 1}/6                            `);
                                    got(1,
                                        {
